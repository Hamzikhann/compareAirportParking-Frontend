...
function AdminLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userRole === "Administrator" || userRole === "Subadmin") {
        ...
        setUserId(user.id);
        setSessionUser(user);
        setUserStore(user, response.data.token);
        toast.success("Admin logged in successfully!");
        // Force a reload to ensure all auth state is properly initialized
        setTimeout(() => {
          navigate("/admin", { replace: true });
          window.location.reload(); // Temporary fix to ensure proper state update
        }, 500);
      } else {
  ...
  const goToForgotPassword = () => {
    navigate("/forgotPassword", { state: { from: "admin" } }); // or "user"
  };
