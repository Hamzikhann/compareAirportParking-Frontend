...
const AdminDashboard = () => {
	...
	const exportBookingData = async () => {
		setExportLoading(true);
		try {
			// Fetch complete booking data for export
			const response = await ApiServices.postRequest({
				path: "booking/list",
				payload: {}
			});
			...
			if (response?.data?.data?.allBookings) {
				...
				// Prepare CSV content with all available fields
				const headers = [
					"Booking ID",
					"Start Date",
					"End Date",
				...
				const csvData = allBookings.map((booking) => [
					...
					booking.startDate,
					booking.endDate,
					booking.startTime || "00:00", // Default to 00:00 if not provided
					booking.endTime || "00:00", // Default to 00:00 if not provided
					booking.airport,
					booking.status,
					booking.registrations?.[0]?.title || "N/A",
					booking.registrations?.[0]
						? `${booking.registrations[0].firstName} ${booking.registrations[0].lastName}`
						: "N/A",
					booking.registrations?.[0]?.email || "N/A",
					booking.registrations?.[0]?.phoneNo || "N/A",
					booking.registrations?.[0]?.outboundTerminal || "N/A",
					booking.registrations?.[0]?.outboundFlightNo || "N/A",
					booking.registrations?.[0]?.inboundTerminal || "N/A",
					booking.registrations?.[0]?.inboundFlightNo || "N/A",
					booking.vehicles?.[0]?.vehicleRegistration || "N/A",
					booking.vehicles?.[0]?.vehicleManufacturer || "N/A",
					booking.vehicles?.[0]?.vehicleModel || "N/A",
					booking.vehicles?.[0]?.vehicleColour || "N/A",
					booking.vehicles?.[0]?.numberofPassengers || "N/A",
					booking.vehicles?.[0]?.type || "N/A",
					booking.package?.title || "N/A",
					booking.package?.price || "N/A",
					booking.userId || "N/A",
					booking.packageId || "N/A"
				]);
				...
				toast.success(`Successfully exported ${allBookings.length} bookings!`);
			} else {
				throw new Error("No booking data available");
			}
		} catch (error) {
			console.error("Error exporting booking data:", error);
			alert("Failed to export booking data. Please try again.");
		} finally {
		...
		},
		{
			...
			cell: (row) => (
				<div className="flex gap-2">
					<button className="text-gray-600 hover:text-indigo-700" title="View" onClick={() => handleViewBooking(row)}>
						<LuView size={18} />
					</button>
		...
		<div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
			...
			<main className="w-full px-4 sm:px-6 lg:px-12 py-12">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6 lg:mb-8">
					{loadingStats ? (
						...
						statsData.map((stat, index) => (
							...
							>
								<div className="flex items-center justify-between mb-3 lg:mb-4">
									<h3 className="text-gray-500 text-xs lg:text-sm font-medium uppercase tracking-wide">{stat.title}</h3>
								...
								<div className="flex items-baseline justify-between">
									...
									<span
										className={`text-xs lg:text-sm font-medium ${stat.isPositive ? "text-green-600" : "text-red-600"}`}
									>
										{stat.change}
									</span>
								</div>
				...
				<div className="grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
					...
					<div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6">
						<h2 className="text-lg font-semibold text-gray-900 mb-4 lg:mb-6">User Distribution</h2>
					...
					<div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6">
						<div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 lg:mb-6 gap-3">
							<h2 className="text-lg font-semibold text-gray-900">Revenue & Users</h2>
							<div className="flex gap-2">
								{["weekly", "monthly"].map((filterOption) => (
									<button
										...
										className={`px-3 py-1 text-sm rounded-md transition-colors ${
											filter === filterOption ? "bg-blue-500 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"
										}`}
									>
			...
			<section className="w-full px-4 sm:px-6 lg:px-12 pb-12">
				<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
					<h1 className="text-2xl font-semibold font-serif text-gray-700">All Bookings</h1>
					<button
						onClick={exportBookingData}
						disabled={exportLoading || bookingData.length === 0}
						className="flex items-center gap-2 bg-gradient-to-r from-[#244c30] to-[#b7e06e] text-white px-4 py-2 rounded-lg hover:shadow-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
					>
						{exportLoading ? (
							<>
							...
							<>
								<LuDownload size={18} />
								<span>Export Data ({bookingData.length})</span>
							</>
						)}
					</button>
				</div>
						...
						<DataTable
							...
							noDataComponent={
								<div className="py-8 text-center">
									...
									<p className="mt-1 text-sm text-gray-500">No bookings available</p>
								</div>
							}
						/>
					)}
										...
										<div className="bg-white rounded-xl shadow-lg border border-gray-100 p-4 sm:p-6">
											...
											<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
												...
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.registrations[0].email}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.registrations[0].phoneNo}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.registrations[0].outboundTerminal}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.registrations[0].outboundFlightNo}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.registrations[0].inboundTerminal}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4 sm:col-span-2">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.registrations[0].inboundFlightNo}
													</p>
												</div>
											</div>
										</div>
										...
										<div className="bg-white rounded-xl shadow-lg border border-gray-100 p-4 sm:p-6">
											...
											<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.vehicles[0].vehicleRegistration}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.vehicles[0].vehicleManufacturer}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.vehicles[0].vehicleModel}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.vehicles[0].vehicleColour}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.vehicles[0].numberofPassengers}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.vehicles[0].type}
													</p>
												</div>
											</div>
										</div>
													...
													<ul className="mt-2 space-y-1">
														{JSON.parse(selectedBooking.package.bulletPoints || "[]").map((point, index) => (
															<li key={index} className="flex items-start space-x-2">
																<svg
																	className="w-3 h-3 sm:w-4 sm:h-4 text-[#244c30] mt-0.5 flex-shrink-0"
																	fill="currentColor"
																	viewBox="0 0 20 20"
																>
																	<path
										...
										<div className="bg-white rounded-xl shadow-lg border border-gray-100 p-4 sm:p-6">
											...
											<div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.bookingAssignments[0].employee?.firstName}{" "}
														{selectedBooking.bookingAssignments[0].employee?.lastName}
													</p>
												</div>
												<div className="bg-gray-50 rounded-lg p-3 sm:p-4">
													<span className="text-xs sm:text-sm font-medium text-gray-500 uppercase tracking-wide">
													...
													<p className="text-base sm:text-lg font-semibold text-gray-900 mt-1">
														{selectedBooking.bookingAssignments[0].status}
													</p>
												</div>
											</div>
										</div>
